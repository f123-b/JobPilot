...
_TRANSIENT_HINTS = (
    "timeout", "timed out", "temporary", "navigation", "network", "connection",
    "element", "not found", "stale", "rate limit", "429", "503", "502",
)
...
def evaluate_run(run: BrowserRunResult) -> AgentEvaluation:
    ...
    score = min(100, score)
    failure = classify_failure(errors)
    retryable = run.success is not True and (
        failure.retryable or any(hint in " ".join(errors).lower() for hint in _TRANSIENT_HINTS)
    )
    ...
    return AgentEvaluation(
        ...
        metrics={
            ...
            "result_chars": len(run.final_result),
            "discovered_jobs": len(run.discovered_jobs),
